...
class Controller():
    ...
    def new_game(self):
        ...
        self.set_reset_button_bindings()
    ...
    def set_reset_button_bindings(self):
    ...
    def update_button_displays(self, updated_squares: list[tuple[int, int]]):
        for square in updated_squares:
            ...
            if (square_state == SquareState.HIDDEN): continue
            if (square_state == SquareState.REVEALED):
                self.game_window.set_square_display_number(column, row, self.model.get_square_value(column, row))
